...
for folder in tqdm(folder_list):
    ...
    efficiency = (
        simulation_settings["simulation"]["duration"] / simulation_settings["process"]["runtime"]
    )
    if behavior not in runtimes:
        runtimes[behavior] = {}
    runtimes[behavior][cars_per_second] = efficiency
...
plt.title("Efficiency of the simulations")
plt.xlabel("Cars per second")
plt.ylabel("Efficiency (duration/runtime)")
for behavior in durations:
...
plt.savefig(os.path.join(tmpfolder, "efficiency.png"))
plt.show()
